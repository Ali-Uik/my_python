...
count = []
with open('users.txt', mode='r', encoding='utf-') as users_file:
...
if len(count) == 1:
    file4 = open(f'{user_name.lower()}{str(user_age)}.txt', mode='r', encoding='utf-8')
    data = file4.read()
    print(data)
    file4.close()
elif len(count) == 0:
    n = user_age - 12
    main_info = qazo(n)
    file = open(f'{user_name.lower()}{str(user_age)}.txt', mode='w', encoding='utf-8')
