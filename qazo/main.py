...
count = []
with open('../qazo/users/users.txt', mode='r', encoding='utf-') as users_file:
...
if len(count) == 1:
    file4 = open(f'../qazo/users/{user_name.lower()}{str(user_age)}.txt', mode='r',
                 encoding='utf-8')
    data = file4.read()
    print(data)
    file4.close()
    ...
    file = open(f'../qazo/users/{user_name.lower()}{str(user_age)}.txt', mode='w',
                encoding='utf-8')
    file.write(main_info)
    file.close()
    file2 = open('../qazo/users/users.txt', mode='a', encoding='utf-8')
